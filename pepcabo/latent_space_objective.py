...
class LatentSpaceObjective:
    ...
    def __init__(
        ...
        self.a_vae = None
        self.initialize_vae()
        assert (self.a_vae is not None) and (self.p_vae is not None)

    def __call__(self, z,y,count_calls=True,force_batch=False,batch_size=None):
        ''' Input 
        ...
        for i in reversed(range(len(decoded_xs))):
            l= len(decoded_xs[i])
            if l>15 or l<5 or (any( (special_char in decoded_xs[i]) for special_char in ['<PAD>','<START>','<STOP>','B','Z','X','*'])):
                v=decoded_xs.pop(i)
                z = torch.cat((z[:i], z[i+1:]))
        ...
        for i,x in enumerate(decoded_xs):
            if x in self.xs_to_scores_dict:
                ...
                query_x.append(x)
                if (batch_size is not None) and len(query_x)==batch_size:
                    bp=i+1
                    break
        decoded_xs, y, z=decoded_xs[:bp], y[:bp], z[:bp]

        if len(query_x):
            score_dict = self.query_oracle(query_x)

        for x in decoded_xs:
            if x in self.xs_to_scores_dict:
                score = self.xs_to_scores_dict[x][0]
                fo = self.xs_to_scores_dict[x][1]
            else:
                self.xs_to_scores_dict[x] = score_dict[x]
                score = self.xs_to_scores_dict[x][0]
                fo = self.xs_to_scores_dict[x][1]
        ...
        if len(fos):
            fos_arr = np.array(torch.stack(fos).squeeze(1))
        else:
            fos_arr = np.zeros((0,3))
        ...
        out_dict['fos'] = fos_arr
        out_dict['valid_zs'] = valid_zs
        out_dict['decoded_xs'] = decoded_xs
        out_dict['tmp_batch'] = len(decoded_xs)
        out_dict['pred']=np.array(y.flatten())
        assert len(valid_zs)==len(decoded_xs)
        return out_dict
